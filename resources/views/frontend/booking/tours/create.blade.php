...
<div class="py-12">
    <x-page-container>
        <div class="mb-6">
            <a href="{{ route('tours.show', $tour) }}" class="text-sm font-medium text-storm-gray hover:text-carbon-black">&larr; Kembali ke Detail Paket</a>
        </div>

        <h1 class="text-3xl font-bold text-carbon-black mb-8">Form Pemesanan Paket Wisata</h1>
            ...
            <div class="flex-grow">
                <form action="{{ route('booking.tours.store', $tour) }}" method="POST" class="bg-canvas-white p-6 md:p-8 rounded-btn-card border border-soft-divider shadow-sm" id="bookingForm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <label class="block text-sm font-medium text-carbon-black mb-2">Tanggal Keberangkatan <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-carbon-black mb-2">Jumlah Peserta <span class="text-red-500">*</span></label>
                            <input type="number" name="participant_count" id="participant_count" value="{{ old('participant_count', 1) }}" min="1" required class="w-full border border-soft-divider rounded-btn p-3 focus:outline-none focus:border-carbon-black" oninput="calculateTotal()">
                            <x-form-error :messages="$errors->get('participant_count')" />
                        </div>
                    </div>

                    <div class="mb-8">
                        <label class="block text-sm font-medium text-carbon-black mb-2">Catatan Khusus</label>
                        <textarea name="note" rows="3" placeholder="Contoh: Ada 1 peserta anak-anak" class="w-full border border-soft-divider rounded-btn p-3 focus:outline-none focus:border-carbon-black">{{ old('note') }}</textarea>
            ...
            <div class="w-full md:w-96 flex-shrink-0">
                <div class="bg-faint-gray p-6 rounded-btn-card border border-soft-divider sticky top-24">
                    ...
                    <div class="flex items-center space-x-4 mb-6 pb-6 border-b border-soft-divider">
                        <div>
                            <div class="font-bold text-carbon-black">{{ $tour->name }}</div>
                            <div class="text-sm text-storm-gray">Durasi: {{ $tour->duration }}</div>
                        </div>
                    </div>
