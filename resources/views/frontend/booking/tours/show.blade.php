            ...
            <div>
                <div class="bg-faint-gray p-6 rounded-btn-card border border-soft-divider sticky top-24">
                    ...
                    <div class="flex justify-between items-center mb-6 pb-6 border-b border-soft-divider">
                        @if($tourBooking->discount_amount > 0)
                            <div class="w-full space-y-2 text-sm">
                                <div class="flex justify-between text-storm-gray">
                                    <span>Harga Asli</span>
                                    <span>Rp {{ number_format($tourBooking->original_price, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between text-green-700 font-medium">
                                    <span>Diskon Kupon <span class="font-mono text-xs bg-faint-gray px-1.5 py-0.5 rounded border border-soft-divider">{{ $tourBooking->coupon_code }}</span></span>
                                    <span>- Rp {{ number_format($tourBooking->discount_amount, 0, ',', '.') }}</span>
                                </div>
                    ...
                    @if($tourBooking->booking_status === 'canceled')
                        <div class="p-4 bg-red-50 text-red-800 rounded-btn text-center font-medium mb-3">
                            Pesanan Dibatalkan
                        </div>
                    @elseif($tourBooking->payment_status === 'paid')
                        <div class="p-4 bg-green-50 text-green-800 rounded-btn text-center font-medium mb-3">
                            ✓ Pembayaran Lunas
                        </div>
                        <a href="{{ route('customer.invoice.download', ['type' => 'tour', 'booking_code' => $tourBooking->booking_code]) }}"
                           target="_blank"
                    ...
                    @elseif(in_array($tourBooking->payment_status, ['unpaid', 'pending']))
                        ...
                        <form id="cancel-form-{{ $tourBooking->booking_code }}" action="{{ route('customer.bookings.cancel', ['type' => 'tour', 'booking_code' => $tourBooking->booking_code]) }}" method="POST">
                            @csrf
                            <button type="button" onclick="openCancelModal('cancel-form-{{ $tourBooking->booking_code }}')" class="w-full flex items-center justify-center gap-2 bg-transparent text-carbon-black border border-soft-divider px-8 py-3 rounded-lg font-medium hover:bg-faint-gray transition-colors">
                                Batalkan Pesanan
                            </button>
                        </form>
                    @else
                        <div class="p-4 bg-red-50 text-red-800 rounded-btn text-center font-medium mb-3">
                            Pembayaran {{ ucfirst($tourBooking->payment_status) }}
                        </div>
                    @endif
                </div>
            </div>
