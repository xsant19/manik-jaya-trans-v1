        ...
        <div class="flex flex-col gap-12 lg:flex-row lg:gap-16">
            ...
            <div class="w-full lg:w-80 xl:w-96 shrink-0">
                <div class="sticky top-24 space-y-4">
                    ...
                    <div class="rounded-card border border-soft-divider bg-faint-gray px-5 py-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-xs leading-relaxed text-storm-gray">
                                    Hubungi kami via WhatsApp untuk pertanyaan seputar kendaraan ini.
                                </p>

                                {{-- Date Input --}}
                                <div class="mt-3 mb-3">
                                    <label for="wa-tanggal" class="block text-xs font-medium text-storm-gray mb-1">Tanggal rencana penggunaan <span class="text-dust-bunny">(opsional)</span></label>
                                    <input type="date" id="wa-tanggal"
                                           min="{{ date('Y-m-d') }}"
                                           class="w-full rounded-btn border border-soft-divider bg-canvas-white px-3 py-2 text-sm text-carbon-black focus:border-carbon-black focus:outline-none" />
                                </div>

                                <a href="#" id="wa-chat-btn"
                                   class="inline-flex items-center gap-2 text-xs font-medium text-carbon-black hover:underline cursor-pointer">
                    ...
                    <script>
                        document.getElementById('wa-chat-btn').addEventListener('click', function(e) {
                            e.preventDefault();
                            var nama = @json($vehicle->name);
                            var phone = @json(config('company.phone_intl'));
                            var tanggal = document.getElementById('wa-tanggal').value;
                            var bulan = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
                            var pesan = 'Halo, saya ingin bertanya tentang ' + nama;
                            if (tanggal) {
                                var d = new Date(tanggal + 'T00:00:00');
                                pesan += ' untuk tanggal ' + d.getDate() + ' ' + bulan[d.getMonth()] + ' ' + d.getFullYear();
                            }
